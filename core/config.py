...
class Config:
    ...
    def _override_with_env(self):
        ...
        if os.getenv("NEO4J_PASSWORD"):
            self.config.setdefault("neo4j", {})["password"] = os.getenv(
                "NEO4J_PASSWORD"
            )
        if os.getenv("NEO4J_DATABASE"):
            self.config.setdefault("neo4j", {})["database"] = os.getenv(
                "NEO4J_DATABASE"
            )
        ...
        if os.getenv("LLM_MODEL_NAME"):
            self.config.setdefault("llm", {})["model_name"] = os.getenv(
                "LLM_MODEL_NAME"
            )
        ...
        if os.getenv("JSEARCH_API_KEY"):
            self.config.setdefault("job_apis", {}).setdefault("jsearch", {})[
                "api_key"
            ] = os.getenv("JSEARCH_API_KEY")
        if os.getenv("REMOTIVE_API_KEY"):
            self.config.setdefault("job_apis", {}).setdefault("remotive", {})[
                "api_key"
            ] = os.getenv("REMOTIVE_API_KEY")
