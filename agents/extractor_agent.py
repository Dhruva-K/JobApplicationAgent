...
class ExtractorAgent(BaseAgent):
    ...
    def __init__(self, graph_memory: GraphMemory, llm_client: LLMClient):
        ...
        """
        super().__init__(
            name="ExtractorAgent", graph_memory=graph_memory, role="extractor"
        )
    ...
    def _process_extracted_data(
        self, job_id: str, extracted: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def batch_extract(
        self, job_ids: List[str], delay_between_jobs: float = 1.0
    ) -> Dict[str, Dict[str, Any]]:
        """Extract information for multiple jobs.

        Args:
            job_ids: List of job identifiers
            delay_between_jobs: Delay in seconds between each job to avoid rate limits
        ...
        for idx, job_id in enumerate(job_ids):
            try:
                ...
                if idx < len(job_ids) - 1 and delay_between_jobs > 0:
                    logger.info(
                        f"[ExtractorAgent] Waiting {delay_between_jobs}s before next extraction (free tier rate limiting)..."
                    )
    ...
    async def _handle_data_request(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Handle job extraction request from another agent.

        Args:
            payload: Request with 'job_id' or 'job_ids'
        ...
        try:
            ...
            if job_id:
                ...
                results = self.batch_extract(job_ids)
                return {"status": "success", "count": len(results), "results": results}
            else:
                return {"status": "error", "error": "job_id or job_ids required"}
        except Exception as e:
            logger.error(f"[ExtractorAgent] Error handling request: {e}")
    ...
    async def _handle_status_update(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        return {"status": "acknowledged", "agent": "extractor"}
